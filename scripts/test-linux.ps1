...
<#
.SYNOPSIS
    Runs GitRun tests inside a Linux container.
...
.DESCRIPTION
    ...
    Unix code path (UnixGitRun / Libc) without leaving their host.
...
$testCmd = "dotnet test --no-build -c $Configuration tests/GitRun.Tests/GitRun.Tests.csproj"
if ($Filter) {
...
# standard `src/GitRun/bin/...` path the test project expects via
# AssemblySearchPaths — just inside the anonymous volume.
$shadowedPaths = @(
    '/src/src/GitRun/bin',
    '/src/src/GitRun/obj',
    '/src/tests/GitRun.Tests/bin',
    '/src/tests/GitRun.Tests/obj'
)
